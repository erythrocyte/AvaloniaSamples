...
public class GameField : PropertyChangedBase
{
    ...
    public GameField(int width, int height)
    {
        ...
        GameObjects.Add(
            Player = new Player(this, new CellLocation(width / 2, height / 2), FacingEnum.East));
        ...
        for (var c = 0; c < 10;)
        {
            ...
            if (!Tiles[x, y].IsPassable)
                continue;
            c++;
            GameObjects.Add(new Tank(this, new CellLocation(x, y), (FacingEnum)Random.Next(4),
    ...
    private TerrainTileTypeEnum GetTypeForCoords(int x, int y)
    {
        if (x / 2 == Width / 4)
